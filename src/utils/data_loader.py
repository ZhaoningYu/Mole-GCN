...
class FileLoader(object):
    ...
    def load_data(self):
        ...
        if self.args.edge_weight:
            with open('../data/%s/edge_labels.txt' % (data), 'r') as f:
                w = f.readlines()
            weights = [int(i) for i in w] # Weight for edges
            print("weights:",len(weights))

        with open('../data/%s/graph_labels.txt' % (data), 'r') as f:
            l = f.readlines()
